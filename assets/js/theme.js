...
var bm3 = {
    bm3Timeout: null,
    ready: function() {
        bm3.setActiveViewModeMediaQuery();
    },
    resize: function() {
        bm3.setActiveViewModeMediaQuery();
    },
    init: function() {
        ...
        this.initLogInPopup();
        this.initSignUpPopup();
        this.initForgotPopup();
    ...
    initSliderBanner: function() {
        ...
        bannerBlock.each(function() {
            var self = $(this),
                bannerItems = self.find(".bm3-row-carousel"),
                ...
                iconNext = '<i class="fa-solid fa-chevron-right"></i>',
            ...
            if (bannerItems.length > 0) {
                if (!bannerItems.hasClass('slick-initialized')) {
                    bannerItems.slick({
                        ...
                        dots: itemDots_mb,
                        arrows: itemRow_mb,
                        nextArrow: '<button type="button" class="slick-arrow slick-next" aria-label="Slide Next">' + iconNext + '</button>',
                        prevArrow: '<button type="button" class="slick-arrow slick-prev" aria-label="Slide Prev">' + iconPrev + '</button>',
                        responsive: [{
                            breakpoint: 1300,
                            settings: {
                                dots: itemDots,
...
initHeaderSticky: function() {
    ...
    if (sticky.length > 0) {
        bm3.stickyScroll(sticky, header_position, header_height, activeClass);
    }
},
...
initDropdown: function() {
    $doc.on('click', '[data-dropdown-toggle]', (event) => {
        if ($(event.currentTarget).find(".bm3-dropdown").hasClass('show')) {
            $('.bm3-dropdown').removeClass('show');
        } else {
            $('.bm3-dropdown').removeClass('show');
            $(event.currentTarget).find(".bm3-dropdown").toggleClass('show');
        }
    });

    $doc.on('click', (event) => {
        if ($('.bm3-dropdown').hasClass('show') && ($(event.target).closest('[data-dropdown-toggle]').length === 0)) {
            $('.bm3-dropdown').removeClass('show');
        }
    });
},
...
initOpenSearchForm: function() {
    ...
    btnClose = $(".background-header-search, .bm3-search-close");
...
initDynamicBrowserTabTitle: function() {
    var pageTitleContent = document.title,
        newPageTitleContent = 'Tu nueva moto te espera ⚡';
...
initSidebarLogin: function() {
    var authIcon = $("[data-open-auth-sidebar]"),
        btnClose = $(".background-overlay, .bm3-sidebar-close");
...
initNotifyInStock: function() {
    var btnNotify = $('.is-notify-me'),
        btnclose = $(".background-overlay-popup, .bm3-popup-close");
...
initNewsLetterPopup: function() {
    var newstterPopup = $(".bm3-newsletter-popup");
...
initLogInPopup: function() {
    ...
    if (btnOpen.length) {
        ...
        btnOpen.on('click', (event) => {
            ...
            $body.addClass('account-login-show');
        });
        ...
        btnClose.on('click', (event) => {
            ...
            if ($body.hasClass('account-login-show')) {
...
initSignUpPopup: function() {
    var btnOpen = $('[data-open-auth-signup]'),
        btnClose = $(".background-overlay-popup, [data-close-account-signup-popup]");
    ...
    if (btnOpen.length) {
        ...
        if ($body.hasClass('auth-sidebar-show')) {
            ...
            $body.addClass('account-signup-show');
    ...
    if ($body.hasClass('account-signup-show')) {
...
initForgotPopup: function () {
        ...
        if (btnOpen.length) {
            ...
            btnOpen.on('click', (event) => {
                ...
                $body.addClass('account-forgot-show');
            ...
            btnClose.on('click', (event) => {
                ...
                if ($body.hasClass('account-forgot-show')) {
                    $body.removeClass('account-forgot-show');
...
initConfirmPopup: function() {
    var btnOpen = $('[data-open-auth-confirm]'),
        btnClose = $(".background-overlay-popup, [data-close-account-confirm-popup]");
    ...
    if (btnOpen.length) {
        ...
        $body.addClass('account-confirm-show');
    });
    ...
    if ($body.hasClass('account-confirm-show')) {
        $body.removeClass('account-confirm-show');
    }
});
}
},
...
initSliderAboutUs: function() {
    ...
    bannerBlock.each(function() {
        var self = $(this),
            bannerItems = self.find(".bm3-row-slider"),
            itemShow = self.data("item-to-show"),
            itemDots = self.data("item-dots"),
            itemDots_mb = self.data("item-dots-mb"),
            itemRow = self.data("item-arrows"),
...
initClickedActiveVideoBanner: function() {
    if ($('[data-video-banner]').length > 0) {
        ...
        videoIcon.each((index, element) => {
            var self = $(element),
            banner = self.parents('.bm3-block-video'),
...
initProductCustomerViewing: function() {
    ...
    if (wrapper.length > 0) {
        ...
        setInterval(function() {
            ...
            wrapper.find('.text').text('[number] clientes están viendo este producto.'.replace('[number]', numbersViewerList[numbersViewerItem]));
        }, timeViewer);
    }
},
...
initProductSizeChart: function() {
    var btnSizeChart = $('[data-open-size-chart-popup]'),
        btnClose = $(".background-overlay, .bm3-popup-close");
...
initProductCompareColor: function() {
    var btnColorCompare = $('[data-open-compare-color-popup]'),
        btnClose = $(".background-overlay, .bm3-popup-close"),
        imageList = $('.bm3-compareColors-image .row');
...
initZoomImage: function() {
    var productZoom = $(".bm3-productView").find('[data-zoom-image]');
...
initProductImageGallery: function() {
    var sliderFor = $(".bm3-productView").find('.productView-for'),
        sliderNav = $(".bm3-productView").find('.productView-nav'),
...
initToggleSidebarMobile: function() {
    var btnClose = $(".background-overlay, .bm3-page-sidebar-close");
...
initBeforeYouLeave: function() {
    var $beforeYouLeave = $('#bm3-leave-sidebar'),
...
initNotification: function() {
    var $notification = $('.bm3-notification-popup'),
        time = $notification.data('time'),
        idleTime = 0,
        btnClose = $(".bm3-popup-close");
...
initProductBUndle: function() {
    var btnChooseOption = $('.bm3-product-bundle .bundle-product-toogle'),
        btnclose = $(".bm3-popup-close");
    ...
    if (btnChooseOption.length) {
        btnChooseOption.on('click', (event) => {
            ...
            $('.bm3-product-bundle .bundlePdItem').removeClass('is-open');
...
initProductVariantChange: function() {
    var wrapper = $('.productView-variants'),
        variantInputItem = wrapper.find('.bm3-product-item-value'),
        variantNameItem = wrapper.find('.' + variantInputItem.attr('name'));

    variantInputItem.on('change', (event) => {
        var self = $(event.currentTarget),
        value = self.val();

        variantNameItem.find("[data-header-option]").text(value);
    });
},
...
initCloseAcceptCookie: function() {
    var cookieElm = $('.bm3-accept-cookie-popup'),
...
intiShowItemGallery: function() {
    var elm = $('.block-custom-instagram'),
        ...
        itemShow = elm.find('.bm3-row-item');
    ...
    if (elm.length > 0) {
        if ($win.width() < 768) {
            ...
            btnLoadMore.on("click", function(e) {
                e.preventDefault();
                elm.find('.bm3-row-item:hidden').slice(0, 2).slideDown();
                if (elm.find('.bm3-row-item:hidden').length === 0) {
                    btnLoadMore.hide();
                }
            });
            ...
            btnLoadMore.on("click", function(e) {
                e.preventDefault();
                elm.find('.bm3-row-item:hidden').slice(0, 3).slideDown();
                if (elm.find('.bm3-row-item:hidden').length === 0) {
                    btnLoadMore.hide();
                }
            });
            ...
            btnLoadMore.on("click", function(e) {
                e.preventDefault();
                elm.find('.bm3-row-item:hidden').slice(0, 5).slideDown();
                if (elm.find('.bm3-row-item:hidden').length === 0) {
                    btnLoadMore.hide();
                }
            });
